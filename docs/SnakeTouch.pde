...
void draw() {  
  ...
  if (!gameover) {  
    ...
    textSize(30); 
    textAlign(CENTER); 
    text("GAME OVER \n Your Score is: "+ x.size() +"\n Click to Begin", width/2, height/3);
  }
}

void mousePressed() { 
  boolean esquerra = map(mouseX, 0, width, 1, 4) < 2;
  boolean dreta = map(mouseX, 0, width, 1, 4) > 3;
  boolean dalt = map(mouseY, 0, height, 1, 4) < 2;
  boolean baix = map(mouseY, 0, height, 1, 4) > 3;
  int newdir = -1;
  if (esquerra && !dalt && !baix) {
    newdir = 3;
  } else if (dreta && !dalt && !baix) {
    newdir = 2;
  } else if (dalt && !esquerra && !dreta) {
    newdir = 1;
  } else if (baix && !esquerra && !dreta) {
    newdir = 0;
  } else if (!dalt && !baix && !esquerra && !dreta && gameover) {
      newGame();
  }
